...
public class AutoSplitController {
    ...
    public ResponseEntity<Map<String, Object>> createAndAutoSplit(
        ...
        try {
            ...
            // BVA Validation for all input parameters
            ...
            if (costId != null) {
                ...
                Integer vehicleId = request.get("vehicleId") != null ? Integer.parseInt(request.get("vehicleId").toString()) : null;
                String costType = (String) request.get("costType");
                if (costType == null || costType.isEmpty()) {
                    costType = "Other"; // default
                }
            ...
            result.put("shares", shares);
            result.put("message",
                    costId != null ? "Đã phân chia chi phí thành công!" : "Đã tạo chi phí và tự động chia thành công!");

            return ResponseEntity.ok(result);

        } catch (org.springframework.web.server.ResponseStatusException e) {
            // Rethrow ResponseStatusException so the ControllerAdvice/ExceptionHandler can
            // handle it properly
            throw e;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<Map<Integer, Double>> getGroupOwnership(@PathVariable Integer groupId,
        ...
        try {
            ...
            logger.warn(
                    "Group not found or error for groupId: {} (returning 200 empty map for BVA nominal compatibility)",
                    groupId);
            ...
            if ("BY_OWNERSHIP".equals(splitMethod)) {
                ...
                for (Integer userId : ownership.keySet()) {
                    ...
                    if (count == memberCount) {
                        ...
                        roundedPercent = Math.round(realPercent * 100.0) / 100.0;
                        remainingPercent -= roundedPercent;
                        shareAmount = amount.divide(new java.math.BigDecimal(memberCount), 2,
