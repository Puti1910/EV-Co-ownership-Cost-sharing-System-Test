...
def create_workbook(api_list, group_name, prefix, output_filename):
    ...
    for i, api in enumerate(api_list):
        api_name, params, total_tc = api[0], api[1], api[2]
        ...
        def add_tc(desc, proc, expected):
            nonlocal tc_count
            tc_id = f"{prefix}_{str(i+1).zfill(2)}_{tc_count:03d}"
            ws.append([tc_id, desc, proc, expected, "Hệ thống sẵn sàng", "", "", "", ""])
    ...
    print(f"Tạo file {output_filename} thành công!")
...
create_workbook(apis_service, "▶ NHÓM 1: RESERVATION SERVICE", "TC_RES", "ECOCSS_BVA_ReservationService.xlsx")
create_workbook(apis_admin, "▶ NHÓM 2: RESERVATION ADMIN SERVICE", "TC_ADM", "ECOCSS_BVA_ReservationAdminService.xlsx")
