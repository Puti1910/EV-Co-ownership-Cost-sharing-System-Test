// ================================
// COST CREATE JAVASCRIPT
// ================================

class CostCreate {
    constructor() {
        this.form = document.getElementById('costForm');
    ...
    async submitForm() {
        ...
        try {
            const response = await fetch('/costs/create', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                this.showErrorMessage('Có lỗi xảy ra khi tạo chi phí!');
            }
        } catch (error) {
    ...
    setLoadingState(loading) {
        ...
        if (submitBtn) {
            if (loading) {
                ...
                submitBtn.innerHTML = '<i class="fas fa-save"></i> <span>Tạo chi phí</span>';
            }
        }
    }
    ...
    showSuccessModal(data) {
        ...
        if (successDetails) {
            successDetails.textContent = `Chi phí "${data.description}" đã được tạo thành công với số tiền ${this.formatCurrency(parseFloat(data.amount))} VNĐ`;
        }
...
function resetForm() {
    ...
    if (form) {
        form.reset();
        window.costCreate.updatePreview();
    }
}

function closeSuccessModal() {
    window.costCreate.closeSuccessModal();
}

// Initialize when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    window.costCreate = new CostCreate();
});
